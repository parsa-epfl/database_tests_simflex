...
int main( int argc, char *argv[] )
{
  ...
  printf("***************************************\n");
  printf("*** ###hard### TPC-C Load Generator ***\n");
  printf("***************************************\n");
...
*/
  ...
  /* alarm initialize */
  time_count = 0;
  int TEMP_time_sec = PRINT_INTERVAL/1000000;
  int TEMP_time_usec = (PRINT_INTERVAL%1000000);
  itval.it_interval.tv_sec = TEMP_time_sec;
  itval.it_interval.tv_usec = TEMP_time_usec;
  itval.it_value.tv_sec = TEMP_time_sec;
  itval.it_value.tv_usec = TEMP_time_usec;
  ...
  counting_on = 1;
  /* wait signal */
  for(i = 0; i < ((measure_time*1000000) / PRINT_INTERVAL); i++ ) {
#ifndef _SLEEP_ONLY_
    pause();
#else
    sleep(PRINT_INTERVAL / 1000000.0 );
    alarm_dummy();
#endif
  }
  counting_on = 0;
